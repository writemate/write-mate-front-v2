...
import DropdownCharacter from "@/components/workspace/character/network/DropdownCharacter";
...
import {
  ...
  RelationCharacterImage,
  RelationCharacterName,
  RelationCharacterDescription,
...
export default function EditRelation<T extends boolean>({
  ...
  return (
    <EditRelationContainer>
        ...
        <RelationCharacterContainer>
          ...
          {isNewMode && (
            <DropdownCharacter
              ...
              selected={selectedCharacter1?.ch_name}
              setSelected={selectCharacter1}
            />
          )}
          <RelationCharacterDescription>
            {selectedCharacter1?.description}
          </RelationCharacterDescription>
        </RelationCharacterContainer>
        <RelationArrowContainer>
          <div>
            ...
            <input
              type="text"
              value={inputRelationLeft}
              onChange={onRelationLeftChange}
              placeholder="관계를 작성해 주세요."
            />
            <RelationToRight />
          </div>
          <div>
            ...
            <input
              type="text"
              value={inputRelationRight}
              onChange={onRelationRightChange}
              placeholder="관계를 작성해 주세요."
            />
            <RelationStick />
          </div>
        ...
        <RelationCharacterContainer>
          ...
          {isNewMode && (
            <DropdownCharacter
              placeholder="인물 선택"
              options={characterList!}
              selected={selectedCharacter2?.ch_name}
              setSelected={selectCharacter2}
            />
          )}
          <RelationCharacterDescription>
            {selectedCharacter2?.description}
          </RelationCharacterDescription>
        </RelationCharacterContainer>
      </RelationContentsContainer>
      <hr />
      <RelationFooter>
        {!isNewMode && (
          <Button onClick={onClickDelete} $background="#F22828" $color="#fff">
            삭제
          </Button>
        )}
        <CancelButton onClick={closeModal}>취소</CancelButton>
        {!isNewMode && (
          <SaveButton onClick={onClickUpdate} $isSavalble={isSavalble}>
            수정
          </SaveButton>
        )}
        {isNewMode && (
          <SaveButton onClick={onClickCreate} $isSavalble={isSavalble}>
            생성
          </SaveButton>
        )}
      </RelationFooter>
