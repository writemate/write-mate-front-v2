...
import { getCharacter, updateCharacterName, updateCharacterRole, updateCharacterGender, 
  updateCharacterBirthday, addKeywordToCharacter, removeKeywordFromCharacter,
...
export function useCharacter(characterId?: string) {
  ...
  const mutateDeleteCharacter = useOnClickUpdate({
    ...
    onMutate: () => {
      ...
      queryClient.setQueryData(workspaceQueryKeys.characterList(workspace_id), (prev: any) => prev.filter((c: any) => c.id !== character_id));
      return { prevData };
    },
  ...
  const onChangeName = useInputLiveUpdate(updateCharacterName(workspace_id, character_id), "캐릭터 이름 변경", "캐릭터 이름 변경에 실패했습니다.");
  const onChangeRole = useInputLiveUpdate(updateCharacterRole(workspace_id, character_id), "역할 변경", "역할 변경에 실패했습니다.");
  const onChangeGender = useInputLiveUpdate(updateCharacterGender(workspace_id, character_id), "성별 변경", "성별 변경에 실패했습니다.");
  ...
  const onClickAddKeywordToCharacter = useOnClickUpdate({
    ...
    onError: (error, variables, context) => {
      queryClient.setQueryData(workspaceQueryKeys.characterDetail(workspace_id, character_id), context?.prevData);
    }
  });
  ...
  const onClickAddCharacteristic = useOnClickUpdate({
    ...
    onMutate: () => {
      ...
      queryClient.setQueryData(workspaceQueryKeys.characterDetail(workspace_id, character_id), (prev: any) => {
        return {
          ...prev,
          characteristic: [...prev.characteristic, { title: "", content: "" }],
        };
      });
      return { prevData };
    },
  ...
  const onClickRemoveCharacteristic = useOnClickUpdate({
    ...
    onMutate: (index: number) => {
      ...
      if(!prevData) return;
      queryClient.setQueryData(workspaceQueryKeys.characterDetail(workspace_id, character_id), 
        { ...prevData, characteristic: prevData.characteristic.filter((_, i) => i !== index) });
      return { prevData };
    },
    onError: (error, variables, context) => {
      queryClient.setQueryData(workspaceQueryKeys.characterDetail(workspace_id, character_id), context?.prevData);
  ...
  const onChangeCharacteristicTitle = useInputLiveUpdate(
    ...
    "특징 제목 변경",
    "특징 제목 변경에 실패했습니다.",
    (value, index) => setCharacteristicList((prev) => prev.map((c, i) => i === index ? { ...c, title: value } : c)),
  ...
  const onChangeCharacteristicContent = useInputLiveUpdate(
    ...
    "특징 내용 변경",
    "특징 내용 변경에 실패했습니다.",
    (value, index) => setCharacteristicList((prev) => prev.map((c, i) => i === index ? { ...c, content: value } : c)),
